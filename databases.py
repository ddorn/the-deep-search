...
class Paragraph(BaseModel):
    ...
    def from_text(cls, podcast_id: int, text: str, paragraph_order: int):
        return cls(podcast_id=podcast_id, hash=txt_hash(text), text=text, paragraph_order=paragraph_order)
...
class Databases:
    ...
    def get_paragraphs(self, hashes: Sequence[str]) -> list[Paragraph]:
        paragraphs = self.sql.execute("SELECT * FROM paragraphs WHERE hash IN (%s)" % ",".join("?" * len(hashes)), hashes).fetchall()
    ...
    def search(self, embedding: list[float], n: int = 10) -> list[Paragraph]:
        response = self.milvus.search(self.collection_name, [embedding], top_k=10, anns_field="vector")
    ...
    def add_paragraphs_sql(self, paragraphs: list[Paragraph]):
        self.sql.executemany("INSERT INTO paragraphs (podcast_id, hash, text, paragraph_order) VALUES (?, ?, ?, ?)", [(p.podcast_id, p.hash, p.text, p.paragraph_order) for p in paragraphs])

    def add_podcast_sql(self, podcast: Podcast):
        res = self.sql.execute("INSERT INTO podcasts (title, filename) VALUES (?, ?)", (podcast.title, podcast.filename))
